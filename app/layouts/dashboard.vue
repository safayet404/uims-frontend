<template>
  <div class="min-h-screen bg-gray-50 flex">
    ...
    <aside
      ...
      <div class="flex items-center gap-3 px-4 py-4 border-b border-gray-100">
        ...
        <button class="ml-auto text-gray-400 hover:text-gray-600 hidden lg:block" @click="ui.toggleCollapse()">
          <UIcon :name="ui.sidebarCollapsed ? 'i-heroicons-chevron-right' : 'i-heroicons-chevron-left'"
            class="w-4 h-4" />
        </button>
      </div>
      ...
      <nav class="flex-1 overflow-y-auto py-4 px-2 space-y-1">
        <template v-for="group in navGroups" :key="group.label">
          <p v-if="!ui.sidebarCollapsed && group.label"
            class="px-3 pt-3 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ group.label }}</p>
          <template v-for="item in group.items" :key="item.to">
            <NuxtLink v-if="canSee(item)" :to="item.to"
      ...
      <div class="border-t border-gray-100 p-3">
        <div class="flex items-center gap-3">
          ...
          <UButton v-if="!ui.sidebarCollapsed" icon="i-heroicons-arrow-right-on-rectangle" size="xs" color="gray"
    ...
    <div
      :class="['flex-1 flex flex-col min-w-0 transition-all duration-300', ui.sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-64']">
      <header class="sticky top-0 z-40 bg-white border-b border-gray-200 h-16 flex items-center px-4 gap-4">
        ...
        <div class="flex items-center gap-2">
          ...
          <UButton icon="i-heroicons-arrow-right-on-rectangle" color="gray" variant="ghost" size="sm"
            @click="handleLogout" />
        </div>
      </header>
...
const navGroups = computed(() => [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Permissions', to: 'permissions', icon: 'i-heroicons-shield-check', permission: 'permissions.read' },
